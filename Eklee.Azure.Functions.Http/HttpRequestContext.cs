	...
	public class HttpRequestContext : IHttpRequestContext
	{
		...
		public Security Security
		{
			get
			{
				if (_security == null)
				{
					_security = new Security { Principal = new Principal() };

					if (Request.Headers.ContainsKey(X_MS_CLIENT_PRINCIPAL_NAME))
					{
						_security.Principal.Name = Request.Headers[X_MS_CLIENT_PRINCIPAL_NAME];
					}

					if (Request.Headers.ContainsKey(X_MS_CLIENT_PRINCIPAL_ID))
					{
						_security.Principal.Id = Request.Headers[X_MS_CLIENT_PRINCIPAL_ID];
					}
				}
